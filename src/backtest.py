"""
Backtesting -- simula a estrategia do bot em dados historicos da Binance.

Como funciona:
- Baixa velas de 1h para cada simbolo
- Para cada vela, calcula os mesmos indicadores tecnicos
...
BACKTEST_DAYS  = 365    # quantos dias para tras simular
...
# bloqueia novas entradas (evita bull extremo)
BULL_FILTER_PCT = 30.0
...
def _is_extreme_bull(df_btc: pd.DataFrame, current_ts) -> bool:
    """Retorna True se BTC subiu mais que BULL_FILTER_PCT nos ultimos 30 dias."""
    try:
        past     = current_ts - pd.Timedelta(days=30)
        slice_df = df_btc[(df_btc.index >= past) & (df_btc.index <= current_ts)]
        if len(slice_df) < 2:
            return False
...
def _check_intracandle_sl_tp(position: dict, low: float, high: float, sl_pct: float) -> tuple[str | None, float]:
    """
    Simula o ciclo rapido usando high/low da vela de 1h.
    Se SL e TP forem tocados na mesma vela, assume SL primeiro (conservador).
    """
...
def run_backtest(symbol: str, df: pd.DataFrame, btc_df: pd.DataFrame | None = None) -> dict:
    ...
    for ts, row in df.iterrows():
        ...
        # Verifica SL/TP intra-vela usando high/low
        if position:
            sl     = SYMBOL_SL.get(symbol, STOP_LOSS_PCT)
            reason, exit_price = _check_intracandle_sl_tp(position, row["low"], row["high"], sl_pct=sl)
            if reason:
        ...
        if symbol != "BTCUSDT" and btc_df is not None and _is_extreme_bull(btc_df, ts):
            equity_curve.append({"ts": ts, "equity": usdt + (price * position["qty"] if position else 0)})
            continue

        # Gera sinal
        trade_signal = generate_signal(row)
        ...
        # Executa sinal
        if trade_signal.signal == "BUY" and trade_signal.confidence >= MIN_CONFIDENCE and position is None and usdt >= 10:
            spend    = min(TRADE_USDT, usdt * 0.99)
            qty      = round(spend / price, 6)
            usdt    -= spend
            position = {"entry": price, "qty": qty, "date": ts}

        elif trade_signal.signal == "SELL" and trade_signal.confidence >= MIN_CONFIDENCE and position and not HOLD_ON_OPEN_POSITION:
...
def print_report(result: dict):
    ...
    max_dd      = ((eq - rolling_max) / rolling_max * 100).min()
