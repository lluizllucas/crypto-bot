"""
Cliente Binance: conexao, consulta de saldo, preco, filtros de par e execucao de ordens.
"""
    ...
    try:
        ...
        for f in info["filters"]:
            if f["filterType"] == "LOT_SIZE":
                ...
                decimals = len(f["stepSize"].rstrip("0").split(
                    ".")[-1]) if "." in f["stepSize"] else 0

            elif f["filterType"] in ("MIN_NOTIONAL", "NOTIONAL"):
                min_notional = float(f.get("minNotional")
                                     or f.get("notional") or 5.0)
    except Exception as e:
...
def get_klines(symbol: str, interval: str, limit: int) -> list:
    """Retorna as ultimas N velas de um par."""
    return client.get_klines(symbol=symbol, interval=interval, limit=limit)


def get_ticker(symbol: str) -> dict:
    """Retorna o ticker 24h de um par."""
