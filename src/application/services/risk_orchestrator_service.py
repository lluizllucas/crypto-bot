...
def _handle_tp(symbol, idx, pos, price, llm_analyze_fn, market_data_fn):
    """Trata TP atingido: agente decide e executa hold/sell internamente."""
    ...
    save_llm_log(
        ...
        response={"tool_called": result.tool_called, "reasoning": result.reasoning},
        process="monitor",
...
def _handle_early_exit(symbol, idx, pos, price, llm_analyze_fn, market_data_fn):
    """Trata preco proximo do SL: agente decide e executa early_exit internamente."""
    if llm_analyze_fn is None or market_data_fn is None:
        return
